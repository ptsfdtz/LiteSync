...
- 当前仍未实现事件驱动增量同步，后续见 `P0-03/P0-05`。
- 运行中可在控制台输入命令：
  - `sync`：触发全部任务立即同步
  - `open`：预留 UI 打开动作（当前仅确认命令）
  - `exit`：安全退出并保存状态快照
...
- 代码实现状态：
  - 已完成：项目结构、配置加载与校验骨架、首次全量同步基础能力、真实文件监听基础能力、事件调度防抖与串行执行基础能力、增量同步基础能力、冲突策略基础能力、软删除回收目录基础能力、日志落盘与错误分级、自启动基础能力、周期校验基础能力、安全退出与状态落盘、后台控制命令通道、多任务同步基础能力、基础启动流程
  - 未完成：托盘可视化交互、恢复与重放、安装升级体验等核心功能
